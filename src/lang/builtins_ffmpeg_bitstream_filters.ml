...
type 'a handler = {
  ...
  latest_keyframe :
    pos:int -> 'a Avcodec.Packet.t -> 'a Ffmpeg_copy_content.latest_keyframe;
}
...
let process (type a) ~put_data ~(lift_data : a lift_data) ~generator
    ({ time_base; params; stream_idx; latest_keyframe } : a handler)
...
let mk_handler (type a) ~stream_idx ~time_base (params : a Avcodec.params) =
  {
    ...
    latest_keyframe = Ffmpeg_copy_content.latest_keyframe params;
  }
        ...
        (fun mode ->
          ...
          Lang.add_operator ~category:`FFmpegFilter
            ...
            (fun p ->
                        ...
                        ( flush,
                          fun frame ->
                            ...
                            Generator.put_video generator
                              (Frame_content.copy (Frame.video frame))
                              0 pos;
                            on_data ~get_filter ~get_handler ~put_data
                              ~lift_data ~generator
                              (Ffmpeg_copy_content.Audio.get_data
                                 (Frame_content.sub (Frame.audio frame) 0 pos))
                        )
                    | `Video | `Video_only ->
                        ...
                        ( flush,
                          fun frame ->
                            ...
                            Generator.put_audio generator
                              (Frame_content.copy (Frame.audio frame))
                              0 pos;
                            on_data ~get_filter ~get_handler ~put_data
                              ~lift_data ~generator
                              (Ffmpeg_copy_content.Video.get_data
                                 (Frame_content.sub (Frame.video frame) 0 pos))
                        )
                in
                function
                | `Frame frame ->
              ...
              let kind = Source.Kind.of_kind source_kind in
              let consumer =
