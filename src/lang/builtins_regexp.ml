...
type regexp = { descr : string; flags : [ `i | `g | `m ] list; regexp : Re.re }

let all_regexp_flags = [`i; `g; `m]
let string_of_regexp_flag = function `i -> "i" | `g -> "g" | `m -> "m"
...
let split_fun ~flags:_ rex =
  Lang_core.val_fun
    ...
    (fun p ->
      let string = Lang_core.to_string (List.assoc "" p) in
      Lang_core.list (List.map Lang_core.string (Re.Pcre.split ~rex string)))
